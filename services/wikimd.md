...
- **`homepage.md` is required** — it's the landing page Wikmd looks for
  by convention; a fresh instance with no pages errors until it exists.
- **Internal git repo**: Wikmd auto-initializes its own git repo inside
  ...
  separate from any project repos — don't `git push` from inside it
  expecting it to reach GitHub unless a remote's been deliberately added.
- **Link syntax**: plain Markdown, linking to the page's filename slug
  (no extension) — `[Display Text](page-slug)`. Not `[[wikilinks]]`.

## Backup

This wiki's content isn't currently included in the
[Service Backup](../storage-backup/service-backup) rollout — it lives on the NAS itself,
not a node's `/opt/data`. Worth deciding whether to add
`/volume1/wiki/wikimd` as its own backup target, or rely on Synology's
own snapshot/backup tools for NAS-local data.
