...
including the [Docker Registry](docker-registry). Runs on its own node,
separate from the Synology NAS.

## Where it lives

- **Host:** separate node from the NAS (not `longsnapper.gs`)
...
http:
  routers:
    registry:
      rule: "Host(`registry.local.gs-playbook.com`)"
      entrypoints:
        - websecure
      tls:
